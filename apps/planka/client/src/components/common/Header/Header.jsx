...
const Header = React.memo(({ onClose }) => {
  ...
  const handleWhatsAppClick = useCallback(() => {
    dispatch(entryActions.openWhatsAppModal());
  }, [dispatch]);

  const NotificationsPopup = usePopup(NotificationsStep, POPUP_PROPS);
  const UserActionsPopup = usePopup(UserActionsStep, POPUP_PROPS);

  // Support custom logo via localStorage (set in Admin → Appearance)
  const customLogoUrl = typeof window !== 'undefined'
    ? localStorage.getItem('planka_logo_url') || null
    : null;
      ...
      <Menu inverted size="large" className={styles.menu}>
        ...
        <Menu.Menu position="right">
          ...
          {user.role === UserRoles.ADMIN && (
            ...
            >
              <Icon fitted name="whatsapp" color="green" />
            </Menu.Item>
          )}
          <NotificationsPopup>
